...
TMP_CONFIG="$TRACELOCK_HOME/runs/configs/eval_gsm8k.$(date +%Y%m%d_%H%M%S).json"
MATERIALIZE_ARGS=(--template "$REPO_ROOT/configs/eval_gsm8k.json" --output "$TMP_CONFIG")
...
PYTHONPATH="$REPO_ROOT${PYTHONPATH:+:$PYTHONPATH}" "$PYTHON" -m tracelock.dream.run_eval_math "${RUN_ARGS[@]}"
